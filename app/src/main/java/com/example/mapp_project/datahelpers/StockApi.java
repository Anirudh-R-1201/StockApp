...
public class StockApi {
    ...
    private Pair<String,String> stockDataApi(String symbol){
        return new Pair<>("https://query1.finance.yahoo.com/v7/finance/quote?formatted=true&lang=en-US&region=IN&symbols="+symbol+"&fields=symbol%2CshortName%2ClongName%2CregularMarketPrice%2CregularMarketChange%2CregularMarketChangePercent",
                STOCK_DATA_API
        );
    }
    private Pair<String,String> trendingApi(int count){
        return new Pair<>(
                "https://query1.finance.yahoo.com/v1/finance/trending/IN?count="+count,
                TRENDING_API
        );
    }
    private Pair<String,String> chartApi(String ticker, Pair<String,String> rangePair){
        return new Pair<>("https://query1.finance.yahoo.com/v8/finance/chart/"+ticker+"?region=IN&lang=en-IN&includePrePost=false&indicators=close&interval="+rangePair.first+"&useYfid=true&range="+rangePair.second,CHART_API);
    }
    private Pair<String,String> searchApi(String args,int queryCount){
        return new Pair<>("https://query2.finance.yahoo.com/v1/finance/search?newsCount=0&enableFuzzyQuery=false&enableEnhancedTrivialQuery=true&region=IN&lang=en-US&q="+args+"&quotesCount="+queryCount,
                SEARCH_API
        );
    }
    private Pair<String,String> dailyGainerApi(int queryCount){
        return new Pair<>("https://query2.finance.yahoo.com/v1/finance/screener/predefined/saved?corsDomain=finance.yahoo.com&formatted=false&lang=en-US&region=IN&scrIds=day_gainers&count="+queryCount,
                DAILY_GAINER_API
        );
    }
    private Pair<String,String> dailyLoserApi(int queryCount){
        return new Pair<>("https://query1.finance.yahoo.com/v1/finance/screener/predefined/saved?corsDomain=finance.yahoo.com&formatted=false&lang=en-US&region=IN&scrIds=day_losers&count="+queryCount,
                DAILY_LOSER_API
        );
    }
